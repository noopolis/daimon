...
export function checkSiblingBuildFreshness(facts: SiblingBuildFacts): SiblingBuildFreshness {
  if (!facts.hasSourceDirectory) {
    return { packageName: facts.packageName, linked: false, sourcesScanned: 0, outputsScanned: 0, ok: true };
  }
  ...
  if (sourceFiles.length === 0) {
    return failure(facts.packageName, sourceFiles.length, outputFiles.length, `linked package ${facts.packageName} scanned zero source files; ${command}`);
  }
  if (outputFiles.length === 0) {
    return failure(facts.packageName, sourceFiles.length, outputFiles.length, `linked package ${facts.packageName} has no emitted JavaScript; ${command}`);
  }
  ...
  if (newestSource.mtimeMs > oldestOutput.mtimeMs) {
    return failure(
      facts.packageName, sourceFiles.length, outputFiles.length,
      `linked package ${facts.packageName} is stale: newest source ${newestSource.path} (${newestSource.mtimeMs}) `
        + `is newer than oldest output ${oldestOutput.path} (${oldestOutput.mtimeMs}); ${command}`
    );
  }
