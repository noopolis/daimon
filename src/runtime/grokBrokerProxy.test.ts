...
test("proxy retries one 401 with refreshed broker bearer and shuts down", async () => {
  ...
  const result = await fetch(`http://127.0.0.1:${proxy.port}/v1/chat/completions`, { method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json", "x-grok-client-version": "1.0.25" }, body: JSON.stringify({ stream: true, messages: [] }) });
...
test("proxy failures expose only a fixed diagnostic", async () => {
  ...
  const token = proxy.capabilities.issue("agent", "turn");
  proxy.registerIsolationGuard("turn", async () => undefined);
  const result = await fetch(`http://127.0.0.1:${proxy.port}/v1/chat/completions`, { method: "POST", headers: { authorization: `Bearer ${token}`, "x-grok-client-version": "1.0.25" }, body: JSON.stringify({ stream: true, messages: [] }) });
