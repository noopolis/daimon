...
test("absence stays absence: an unopened turn observes undefined, a turn that called nothing observes zero", () => {
  ...
  log.open("turn");
  assert.deepEqual(log.observe("turn"), { started: 0, answered: 0, undecoded: 0, outstanding: [], tunnels: NO_TUNNEL });
...
test("a body the facade could not read counts as undecoded, never as a call with a name", () => {
  ...
  assert.deepEqual(log.observe("turn"), { started: 0, answered: 0, undecoded: 2, outstanding: [], tunnels: NO_TUNNEL });
