...
import { createGrokWorkerIsolationGuard,GrokWorkerAttestationFailure,prepareGrokWorkerAttestation } from "./grokWorkerAttestation.js";
...
export async function startGrokEngineBroker(options: Readonly<{ grokCommand: string; nativeClient: string; credentialHome: string; turnStore: string; registrations: readonly GrokEngineBrokerRegistration[]; usageLedgerPath?: string; modelPolicy?: GrokBrokerModelPolicy }>) {
  // One declared model/effort drives both the worker config bytes the broker attests and the bodies the proxy forwards.
  const modelPolicy = parseGrokBrokerModelPolicy(options.modelPolicy ?? {}); const configSha256 = grokBrokerWorkerConfigSha256(modelPolicy);
  ...
  return {
    async turn(agentId: string, wakeId: string, prompt: string, mcpEndpoint: string, signal?: AbortSignal): Promise<Readonly<{text:string;workerPid:number;workerUid:number;workerStartTime:string}>> {
      ...
      const turnId = createHash("sha256").update(`${agentId}\0${wakeId}`).digest("hex"); const request = { version: "noopolis.daimon.engine-broker.v1", kind: "start_turn", requestId: randomUUID(), turnId, agentId, wakeId, prompt,mcpEndpoint } as const;
    ...
    async close(): Promise<void> { if (closed) return; closed = true; const running=[...active.values()];for (const entry of running) entry.controller.abort();await Promise.allSettled(running.map((entry)=>entry.done));const results=await Promise.allSettled([mcp.close(),proxy.close(),lease.close()]);const failures=results.flatMap((entry)=>entry.status==="rejected"?[entry.reason]:[]);if(failures.length)throw new AggregateError(failures,"engine broker shutdown failed"); },
