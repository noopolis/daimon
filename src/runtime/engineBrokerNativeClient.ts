...
export function decodeNativeBrokerResult(output:Buffer,turnId:string,secrets:readonly string[]=[]):NativeBrokerTurnResult{
  ...
  const reason=workerReason(output.subarray(ENGINE_BROKER_NATIVE_RESULT_BYTES+length),secrets);
 ...
 * for a failure, so this bounded tail is the only account of why it failed.
 ...
 */
function workerReason(tail:Buffer,secrets:readonly string[]):string|undefined{
  if(tail.length===0)return undefined;
  const flattened=tail.toString("utf8").replace(/[\u0000-\u001f\u007f]+/gu," ").replace(/\s+/gu," ").trim();
  const reason=redactCredentialText(flattened,secrets,CLI_ENGINE_MAX_DIAGNOSTIC_BYTES).trim();
