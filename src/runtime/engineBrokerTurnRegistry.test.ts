...
test("turn registry replays terminal results across restart and rejects conflicts", async () => {
  ...
  try {
    ...
    const response = { version: "noopolis.daimon.engine-broker.v2", kind: "completed", requestId: "request-1", turnId: "turn-1", text: "done", workerPid: 11, workerUid: 2200, workerStartTime: "123", outcome: "completed", usage: null, model: "grok-4.6", requests: 1, limitReason: "none" } as const;
    await first.finish(start(), response);
    assert.deepEqual(await new EngineBrokerTurnRegistry(root,"boot-b").begin(start(),"grok-4.6"), { replay: response, ledger: { usage: null, requests: "" } });
...
test("a v1 record sealed before the upgrade still replays, upgraded with no usage and never re-metered", async () => {
  await withRoot(async (root) => {
    ...
    assert.deepEqual(await new EngineBrokerTurnRegistry(root, "boot-b").begin(start(), "grok-4.5"), { replay: { ...v1, version: "noopolis.daimon.engine-broker.v2", outcome: "completed", usage: null, model: "grok-4.5", requests: 0, limitReason: "none" }, ledger: { usage: null, requests: "" } });
  });
});
...
test("the v2 record parser is strict: an unknown member or a v1 frame inside a v2 record is refused", async () => {
  await withRoot(async (root) => {
    ...
    assert.deepEqual(record.ledger, { usage: null, requests: "" });
...
test("sealed ledger bytes must be this turn's own rows and agree with the sealed usage", async () => {
  await withRoot(async (root) => {
    const registry = new EngineBrokerTurnRegistry(root, "boot-a");
    assert.equal(await registry.begin(start(), "grok-4.6"), "start");
    const turnId = "turn-1";
    const usage = { input: 3, cacheRead: 2, cacheWrite: 0, output: 1, total: 6 };
    const response = { version: "noopolis.daimon.engine-broker.v2", kind: "completed", requestId: "request-1", turnId, text: "done", workerPid: 11, workerUid: 2200, workerStartTime: "123", outcome: "completed", usage, model: "grok-4.6", requests: 1, limitReason: "none" } as const;
    const line = (turn: string) => `${JSON.stringify({ v: "noopolis.daimon.turn-usage.v1", turn, total: 6 })}\n`;
    await registry.finish(start(), response, { usage: line(turnId), requests: "" });
    assert.deepEqual(await new EngineBrokerTurnRegistry(root, "boot-b").begin(start(), "grok-4.6"), { replay: response, ledger: { usage: line(turnId), requests: "" } });
    for (const ledger of [{ usage: line("other-turn"), requests: "" }, { usage: null, requests: "" }, { usage: line(turnId), requests: "not json\n" }, { usage: line(turnId), requests: "", extra: 1 }]) {
      await registry.finish(start(), response, ledger as never);
      await assert.rejects(new EngineBrokerTurnRegistry(root, "boot-c").begin(start(), "grok-4.6"), /registry unavailable/u, JSON.stringify(ledger));
    }
  });
});
