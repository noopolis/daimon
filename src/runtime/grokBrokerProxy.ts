...
/** `policy` is the declared model/effort every forwarded body must carry (closed list; defaults grok-4.6/low). */
export async function startGrokBrokerProxy(authority: GrokBrokerCredentialAuthority, upstream: GrokBrokerUpstream = defaultUpstream, policy: GrokBrokerModelPolicy = DEFAULT_GROK_BROKER_MODEL_POLICY): Promise<Readonly<{ port: number; capabilities: EngineBrokerCapabilities; registerIsolationGuard(turnId:string,guard:()=>Promise<void>):void; revokeIsolationGuard(turnId:string):void; close(): Promise<void> }>> {
  const declared = parseGrokBrokerModelPolicy(policy); const capabilities = new EngineBrokerCapabilities();
  const guards=new Map<string,()=>Promise<void>>();const server = createServer((request, response) => { void serve(request, response, authority, upstream, capabilities,guards,declared); });
...
async function serve(request: IncomingMessage, response: ServerResponse, authority: GrokBrokerCredentialAuthority, upstream: GrokBrokerUpstream, capabilities: EngineBrokerCapabilities,guards:Map<string,()=>Promise<void>>,policy:GrokBrokerModelPolicy): Promise<void> {
  try {
    ...
    let token = await authority.accessToken(false);const rejectedDigest=createHash("sha256").update(token).digest("hex"); let prepared = authorizeGrokBrokerProxyRequest({ method: request.method ?? "", pathname: new URL(request.url ?? "/", "http://127.0.0.1").pathname, headers, body }, capabilities, token, policy); token = "";
